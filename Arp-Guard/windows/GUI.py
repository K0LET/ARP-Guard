...
class GUI:
    ...
    def set_toplevel(self):
        ...
        scroll_frame = customtkinter.CTkScrollableFrame(self.toplevel, width=250, height=230)
        ...
        ip = customtkinter.CTkLabel(scroll_frame,
                                    text=f"{self.spoof_ips[0]}",
                                    text_color="white",
                                    font=customtkinter.CTkFont(size=15),
                                    fg_color="transparent").place(relx=0.1, rely=0.9, anchor=CENTER)
        mac = customtkinter.CTkLabel(scroll_frame,
                                     text=f"{self.sd.ip_mac_dict[self.spoof_ips[0]]}",
                                     text_color="white",
                                     font=customtkinter.CTkFont(size=15),
                                     fg_color="transparent").pack()
        scroll_frame.place(relx=0.17, rely=0.3)
    ...
    def detect_mac_thread(self):
        self.sd.ip_mac_dict = {"999.999.999.999": "AA:BB:CC:DD:EE:FF"}
        self.spoof_ips.append("999.999.999.999")
        self.show_spoof_bt.place(relx=0.5, rely=0.9, anchor=CENTER)
        # flag, self.spoof_ips = self.sd.detect_mac()
        # if len(self.spoof_ips) > 1:
        #     pass
        # elif len(self.spoof_ips) == 1:
